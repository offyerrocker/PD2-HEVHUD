...
function HEVHUDPresenter:animate_show_text(params)
	...
	local function cb_animate_fadeout(o)
		local alpha_duration = 2
		o:animate(AnimateLibrary.animate_alpha_lerp,nil,alpha_duration,nil,0)
	end
	...
	if title then 
		...
		self._title:animate(AnimateLibrary.animate_text_mission,cb_animate_fadeout,utf8.to_upper(title),duration,col_1,col_2,nil)
	else
		self._title:clear_range_color(0,1)
	end
	
	
	local function cb_done(o)
		cb_animate_fadeout(o)
		self:_present_done()
	end
	...
	self._desc:animate(AnimateLibrary.animate_text_mission,cb_done,utf8.to_upper(params.text or ""),duration,col_1,col_2,nil)
end
